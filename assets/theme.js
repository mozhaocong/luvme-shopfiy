    ...
    theme.loadCarousels = function (container) {
        return $('.carousel', container).each(function () {
            ...
            if (fixedMode) {
                ...
                console.log('desktopNumPerRow', desktopNumPerRow)
                carouselOptions = {
            ...
            var loadCarousel = function loadCarousel() {
                ...
                console.log('owlCarousel3', carouselOptions)
            };
            loadCarousel();
        });
    };
        ...
        this.functions = {
            ...
            ajaxLoadUrl: function ajaxLoadUrl(url, noPushState) {
                ...
                this.currentAjaxLoadUrlFetch = $.get(url, function (data) {
                    ...
                    console.log('ajaxLoadUrl', this)
                    if(window.setGoogleBuried) {
                        window.setGoogleBuried({type:'ajaxLoadUrl', data: this })
                    }
                }.bind(this));
            } };
        ...
        init: function init(options) {
            ...
            if ($mobileDrawer.data('mobile-expand-with-entire-link')) {
                $mobileDrawer.on('click', '.navigation__item--with-children > .navigation__link', function (evt) {
                    if(window.setGoogleBuried) {
                        window.setGoogleBuried({type:'view_promotion', dom:[evt.target.parentNode]})
                    }
                    console.log('$mobileDrawer 1', evt.target.parentNode)
                    evt.preventDefault();
            ...
            $mobileDrawer.on('click', '.mobile-nav-back', function (evt) {
                console.log('$mobileDrawer 2', evt)
                evt.preventDefault();
            ...
            $mobileDrawer.on('click', '.navigation__tier-2 > .navigation__item > .navigation__children-toggle', function (evt) {
                console.log('$mobileDrawer 1', evt)
                evt.preventDefault();
            ...
            $nav.on('mouseenter mouseleave', '.navigation__tier-1 > .navigation__item--with-children', function (evt, data) {
                ...
                if (evt.type == 'mouseenter') {
                    ...
                    var newNavOpenTimeoutId = setTimeout(function () {
                        ...
                        if(window.setGoogleBuried) {
                            window.setGoogleBuried({type: 'view_promotion', dom: $dropdownContainer})
                        }
                    }, timeoutDelay);
            ...
            var touchHandler = function touchHandler(evt) {
                console.log('touchHandler touchHandler touchHandler', evt)
                if ($(window).width() > 767) {
   ...
   */
    ...
    theme.HeaderSection = new function () {
        this.onSectionLoad = function (container) {
            ...
            $(container).on('click.headerSection', '.show-search-link, .main-search__close', function (e) {
                console.log('click.headerSection')
                ...
                if(window.setGoogleBuried) {
                    window.setGoogleBuried({type:'search_show'})
                }
                return false;
            });
                            ...
                            currReqObj = $.ajax({
                                ...
                                success: function success(response) {
                                    ...
                                    if (areProducts || arePages) {
                                        ...
                                        $('<a class="main-search__results-all-link button altcolour" onclick="heteAllSearchInput()">').
                                        attr('href', linkURL).
                                        ...
                                        theme.initAnimateOnScroll();
                                        if(window.setGoogleBuried) {
                                            window.setGoogleBuried({type:'view_item_list51', data: valueToSearch})
                                        }
                                    } else {
    ...
    theme.SlideshowSection = new function () {
        ...
        this.onSectionLoad = function (container) {
            $('.slideshow', container).each(function () {
                setTimeout(() => {
                    if(window.setGoogleBuried) {
                        window.setGoogleBuried({type: 'view_promotion86'})
                    }
                },10)
                ...
                on('beforeChange', function (event, slick, currentSlide) {
                    ...
                    if(window.setGoogleBuried) {
                        window.setGoogleBuried({type: 'view_promotion86_afterChange', dom: event})
                    }
                });
            });
    ...
    $(function ($) {
        ...
        $(document).on('click', '.mobile-nav-toggle', function (e) {
            if(window.setGoogleBuried) {
                window.setGoogleBuried({type:'navigation_recommendation'})
            }
            console.log('mobile-nav-toggle mobile-nav-toggle')
            e.preventDefault();
